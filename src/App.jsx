...
function Dashboard() {
  ...
  const budgetPercent = budget
    ? Math.min(100, (monthSummary.expense / budget) * 100)
    : 0;
  ...
  return (
    <Screen>
      <View style={styles.brandHeader}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.brandTitle}>Daily Expanse</Text>
          <Text style={styles.brandSubtitle}>Simple money. Clear mind.</Text>
        </View>
        <Pressable
      ...
      <View style={styles.balanceCard}>
        ...
        <View style={styles.balanceRow}>
          <View style={styles.balanceStat}>
            ...
            <Text style={styles.income}>{money(summary.income)}</Text>
          </View>
          <View style={styles.balanceStat}>
            <View style={[styles.dot, { backgroundColor: colors.red }]} />
            <Text style={styles.balanceLabel}>Expense</Text>
            <Text style={styles.expense}>{money(summary.expense)}</Text>
          </View>
        </View>
      </View>
      ...
      <View style={styles.monthCard}>
        <View style={styles.monthHeader}>
          <View>
            ...
            <Text style={styles.monthTitle}>Monthly Spending</Text>
          </View>
        ...
        <View style={styles.monthNumbers}>
          <View style={styles.numberBlock}>
            ...
            <Text style={styles.monthExpense}>
              {money(monthSummary.expense)}
            </Text>
          </View>
          <View style={styles.numberBlock}>
            <Text style={styles.smallLabel}>Income</Text>
            <Text style={styles.monthIncome}>{money(monthSummary.income)}</Text>
          </View>
          {budget > 0 && (
            <View style={styles.numberBlock}>
              <Text style={styles.smallLabel}>Left</Text>
              <Text style={styles.monthLeft}>{money(remaining)}</Text>
            </View>
          )}
        </View>
        {budget > 0 ? (
          <>
            ...
            <Text style={styles.progressText}>
              {Math.round(budgetPercent)}% of {money(budget)} budget used
            </Text>
          </>
        ) : (
          <Pressable
          ...
          >
            ...
            <Text style={styles.link}>Set a monthly budget</Text>
...
function AddTransaction() {
  ...
  useEffect(
    () => setCategoryId(cats.find((x) => x.type === type)?.id || ""),
    [type, cats],
  );
  ...
  const save = async () => {
    ...
    if (!categoryId)
      return Alert.alert("Select category", "Please choose a category.");
    if (!paymentMethodId)
      return Alert.alert(
  ...
  return (
    <Screen keyboard>
      ...
      <View style={styles.segment}>
        ...
        >
          <Text style={type === "expense" ? styles.redText : styles.muted}>
            Expense
          </Text>
        </Pressable>
        <Pressable
        ...
        >
          <Text style={type === "income" ? styles.greenText : styles.muted}>
            Income
          </Text>
        </Pressable>
      </View>
      ...
      <View style={styles.chips}>
        {payments.map((p) => (
          <Pressable
            ...
            style={[styles.chip, paymentMethodId === p.id && styles.chipActive]}
          >
            <Text
              style={
      ...
      <Field
        ...
        placeholder="Optional: lunch, bus fare, salary..."
      />
      <Pressable
        disabled={saving}
...
function Transactions() {
  ...
  const filtered = useMemo(() => {
    ...
    return txs.filter((tx) => {
      ...
      const text =
        `${tx.note || ""} ${tx.amount} ${tx.categoryId || ""} ${tx.paymentMethodId || ""}`.toLowerCase();
    ...
    <SafeAreaView style={styles.safe}>
      <View style={styles.listContainer}>
        <View style={styles.pageHeader}>
          ...
          <Pressable
            onPress={() =>
              Alert.alert("Filter transactions", "Choose a transaction type.", [
                { text: "All", onPress: () => setFilter("all") },
                { text: "Expense", onPress: () => setFilter("expense") },
                { text: "Income", onPress: () => setFilter("income") },
                { text: "Cancel", style: "cancel" },
              ])
            }
            style={styles.iconButton}
          >
            <SlidersHorizontal size={21} color={colors.muted} />
          </Pressable>
        </View>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TransactionRow tx={item} onDelete={() => remove(item.id)} />
            )}
          />
        )}
        <Text style={styles.hint}>
          Use the trash button on a transaction to delete it.
        </Text>
      </View>
    </SafeAreaView>
  );
      ...
      <View style={styles.listContainer}>
        <View style={styles.pageHeader}>
          <View>
            ...
            <Text style={styles.subtle}>
              Keep quick ideas, reminders & details.
            </Text>
          </View>
          <Pressable
    ...
    <Screen keyboard>
      <View style={styles.editorHeader}>
        <View>
          ...
          <Text style={styles.subtle}>Write something you want to keep.</Text>
        </View>
        <Pressable
...
function Statistics() {
  ...
  return (
    <Screen>
      ...
      <Text style={styles.statsSubtitle}>Current month overview</Text>
      <View style={styles.statCard}>
        <View style={styles.statIcon}>
          <TrendingUp size={19} color={colors.green} />
        </View>
        <View>
          <Text style={styles.statTitle}>Income</Text>
          <Text style={styles.statValue}>{money(income)}</Text>
        </View>
      </View>
      <View style={styles.statCard}>
        <View style={styles.statIcon}>
          <TrendingDown size={19} color={colors.red} />
        </View>
        <View>
          <Text style={styles.statTitle}>Expense</Text>
          <Text style={styles.statValue}>{money(expense)}</Text>
        </View>
      </View>
      <View style={styles.statCard}>
        <View style={styles.statIcon}>
        ...
        <View>
          <Text style={styles.statTitle}>Net</Text>
          <Text style={styles.statValue}>{money(income - expense)}</Text>
        </View>
      </View>
      ...
      <View style={styles.categoryCard}>
        {groups.length === 0 ? (
          ...
          groups.map((g) => (
            <View key={g.id} style={styles.categoryItem}>
              <View style={styles.categoryTop}>
                <Text style={styles.categoryName}>{g.name}</Text>
                <Text style={styles.categoryAmount}>{money(g.amount)}</Text>
              </View>
              <View style={styles.progressTrack}>
                <View
                  style={[
                    styles.progressFill,
                    {
                      width: `${expense ? Math.min(100, (g.amount / expense) * 100) : 0}%`,
                    },
                  ]}
                />
              </View>
...
function SettingsScreen() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setBudget(b ? String(b) : "");
    })();
  }, []);
  const save = async () => {
    try {
      await setMonthlyBudget(Number(budget) || 0);
      setSaved(true);
  ...
  return (
    <Screen>
      ...
      <Text style={styles.subtitle}>Personalize your monthly money plan.</Text>
      <View style={styles.settingsCard}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.settingHint}>
            Used to track your spending progress.
          </Text>
        </View>
      </View>
      <Field
      ...
      <View style={styles.infoCard}>
        <Text style={styles.infoTitle}>Daily Expanse</Text>
        <Text style={styles.infoText}>
          Your data is stored locally on this device. No account or server is
          required.
        </Text>
      </View>
    </Screen>
    ...
    <View style={styles.transactionRow}>
      <View
        style={[
          ...
          {
            backgroundColor:
              tx.type === "income" ? colors.greenSoft : colors.redSoft,
          },
        ]}
      >
        {tx.type === "income" ? (
      ...
      <View style={styles.txRight}>
        <Text style={tx.type === "income" ? styles.income : styles.expense}>
          {tx.type === "income" ? "+" : "-"}
          {money(tx.amount)}
        </Text>
        {onDelete && (
    ...
    <Tab.Navigator
      screenOptions={{
        ...
        tabBarStyle: {
          ...
          paddingTop: 6,
          paddingBottom: Platform.OS === "ios" ? 21 : 7,

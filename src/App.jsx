...
function Dashboard() {
  ...
  return (
    <Screen>
      ...
      />
      {txs.length === 0 ? (
        <Empty text={"কোনো transaction নেই।\nআজকের প্রথম খরচটি যোগ করুন।"} />
      ) : (
...
function Transactions() {
  const [txs, setTxs] = useState([]);
  const load = async () =>
    setTxs((await getTransactions()).sort((a, b) => b.date - a.date));
  useEffect(() => {
    load();
  }, []);
  const remove = (id) =>
  ...
  return (
    <Screen>
      ...
      <View style={styles.setting}>
        <Text style={styles.settingTitle}>Daily Expanse</Text>
        <Text style={styles.muted}>Offline expense tracker</Text>
      </View>
      <View style={styles.setting}>
        <Text style={styles.settingTitle}>Storage</Text>
        <Text style={styles.muted}>
          All transactions are stored locally on this device.
        </Text>
      </View>
    </Screen>
  );
  ...
  if (!ready)
    return (
      <View style={styles.loading}>
        ...
        <Text style={styles.muted}>Loading Daily Expanse...</Text>
      </View>
    );
  return (
    <>
